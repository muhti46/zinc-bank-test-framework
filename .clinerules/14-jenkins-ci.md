...
`typecheck` → `npm test` → `report:generate`), secrets policy (only
`credentials('<id>')` references — never hardcode `.env` values), Windows
`bat`/`nodejs` tooling rules, and artifact rules (`reports/**`,
`test-results/**` archived in `post { always }`). A Jenkins failure whose root
cause is a flaky test is a **Healer ticket** — hand off rather than weakening
the pipeline. YAML/Groovy must stay valid; never print or commit credentials.
